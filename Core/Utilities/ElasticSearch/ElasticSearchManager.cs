...
namespace Core.Utilities.ElasticSearch
{
    public class ElasticSearchManager : IElasticSearch
    {
        ...
        public async Task<IResult> CreateNewIndexAsync(IndexModel indexModel)
        {
            ...
            {
                return new Results.Result(success: false, message: "Index already exist");
            }

            var response = await elasticClient.Indices.CreateAsync(indexModel.IndexName, se => se.Settings(
                a => a.NumberOfReplicas(indexModel.NumberOfReplicas).NumberOfShards(indexModel.NumberOfShards)).Aliases(x => x.Alias(indexModel.AliasName)));

            return new Results.Result(success: response.IsValid, message: response.IsValid ? "Success" : response.ServerError.Error.Reason);
        }

        public async Task<IResult> DeleteElasticAsync(ElasticSearchModel model)
        {
        ...
        public async Task<List<ElasticSearchGetModel<T>>> GetAllSearch<T>(SearchParameters parameters) where T : class
        {
            ...
            var searchsResponse = await elasticClient.SearchAsync<T>(s => s.Index(Indices.Index(parameters.IndexName)).From(parameters.From).Size(parameters.Size));
            var list = searchsResponse.Hits.Select(x => new ElasticSearchGetModel<T>()
            {
                ElasticId = x.Id,
                Item = x.Source
            }).ToList();
        ...
        public async Task<List<ElasticSearchGetModel<T>>> GetSearchByField<T>(SearchByFieldsParameters fieldParameters) where T : class
        {
            var elasticClient = GetElasticClient(fieldParameters.IndexName);
            var searchResponse = await elasticClient.SearchAsync<T>(s => s.AllIndices().From(fieldParameters.From).Size(fieldParameters.Size).Query(q => q.Match(m =>
                                                                                                                                                                m.Field(fieldParameters.FieldName)
        ...
        public async Task<IResult> UpdateByElasticAsync(ElasticSearchInsertUpdateModel model)
        {
            var elasticClient = GetElasticClient(model.IndexName);
